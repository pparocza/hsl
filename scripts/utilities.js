...
function bufferGraph(buffer, channel, tag){
	...
	for(var i=0; i<a.length; i++){
		a[i] = 100-(0.5*(100+(Math.floor(a[i]*100))));
	}
...
Sequence.prototype = {
	...
	// add each value in the array to the preceding value
	sumSequence: function(){
	...
	ramp: function(length, upEnd, downStart, upExp, downExp){
	...
	segments: function(length, segmentPointsArray, segmentValuesArray){
	...
	multiplySequence: function(newSequence){
	...
	divideSequence: function(newSequence){
	...
	addSequence: function(newSequence){
	...
	subtractSequence: function(newSequence){
